  ...
  <div id="toper" :class="store.footerBlur ? 'blur' : null">
    <Transition name="fade" mode="out-in">
      <div class="power">
        ...
        <div id="clock">
          <!-- 翻页时钟 -->
          <div :class="store.detectBrowser=='Edge' ? 'date-main-top-edge' : 'date-main-top'" @click="changeFont">
            ...
            <!-- <span>北京时间：&nbsp;</span> -->
              <div  @mouseover="isHovered = true"
               ...
               :class="{ 'scaled-up': isHovered }" class="scaled-one">
               &nbsp;
               <span>{{ currentTime.year }}&nbsp;.&nbsp;</span>
               <span>{{ currentTime.month }}&nbsp;.&nbsp;</span>
               <span>{{ currentTime.day }}&nbsp;&nbsp;</span>
            </div>  
              <div :class=" currentFont == 'Ruthligos' ? 'sm-hidden-special' : 'sm-hidden'"  :style="{ fontFamily: currentFont }">
                {{ currentFont == 'Ruthligos'?currentTime.weekdayEn:currentTime.weekdayAllBig }}
              </div>
            </div>
...
import $ from 'jquery';
import Potatocutlcb from '../../public/font/potatocutlcb-webfont字体/potatocutlcb-webfont.ttf';
import Anurati from '../../public/font/anurati_downcc/Anurati-Regular.otf';
import Ruthligos from '../../public/font/ruthligos/Ruthligos.ttf';
// 定义字体列表
const fonts = ['Anurati', 'Potatocutlcb', 'Ruthligos'] // A、B、C 三种字体
const isHovered = ref(false);
// 当前字体索引
const currentIndex = ref(0)

// 计算当前字体
const currentFont = computed(() => fonts[currentIndex.value])

// 切换字体函数
const changeFont = () => {
  currentIndex.value = (currentIndex.value + 1) % fonts.length
}
$(document).ready(function () {
  ...
  var loop = setInterval(function () {
    ...
    columnsA.forEach(function (ele, i) {
      var n = +cA[i];
      var offset = -n * sizeA;
      ele.style.transform = 'translateY(calc(20vh + ' + offset + 'px - ' + sizeA / 2  + 'px))';
...
@font-face{
	font-family: Potatocutlcb;
	src: url(../../public/font/potatocutlcb-webfont字体/potatocutlcb-webfont.ttf)
  }
  @font-face{
	font-family: Anurati;
	src: url(../../public/font/anurati_downcc/Anurati-Regular.otf)
  }
  @font-face{
	font-family: Ruthligos;
	src: url(../../public/font/ruthligos/Ruthligos.ttf)
  }
  #toper {
    width: 100%;
    position: absolute;
  ...
  * {
  ...
  .colon {
    ...
    transition: transform 300ms;
    transition: transform 300ms, -webkit-transform 300ms;
    transform: translateY(calc(20vh - 43px));
  }
  ...
  @media (min-width: 2552px) {
    ...
    .date-main-top,.date-main-top-edge{
      ...
      span{
      font-family: Potatocutlcb;
      color: rgba(221, 217, 166, 0.88);
      }
      .sm-hidden, .sm-hidden-special {
        position: absolute;
        top: 1vw;
        z-index: -1;
        font-size: 140px;
        color: rgba(135, 135, 202, 0.8);
      }
      .sm-hidden-special {
        top: -0.5vw;
      }
    ...
    .scaled-one{
      display: inline-block; /* 确保 transform-origin 准确作用 */
      transform-origin: center ; /* 从底部中心 */
      /* 关键：transition 在基础类上 */
      transition: transform 0.8s cubic-bezier(0.25, 0.8, 0.25, 1);
      /* cubic-bezier 可以创建更自定义的缓动效果 */
